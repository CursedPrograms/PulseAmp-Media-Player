// ─── converter.cpp ────────────────────────────────────────────────────────────
#include "converter.h"
#include <iostream>
#include <cstring>
#include <algorithm>
#include <filesystem>

extern "C" {
#include <libavformat/avformat.h>
#include <libavcodec/avcodec.h>
#include <libavutil/avutil.h>
#include <libavutil/audio_fifo.h>
#include <libavutil/opt.h>
#include <libswscale/swscale.h>
#include <libswresample/swresample.h>
}

namespace fs = std::filesystem;

Converter::Converter() = default;

void Converter::cancel() {
    cancel_.store(true);
    if (thread_.joinable()) thread_.join();
    cancel_.store(false);
    running_.store(false);
}

void Converter::start(const ConvertJob& job,
                      std::function<void(ConvertProgress)> progress_cb) {
    cancel();
    running_.store(true);
    cancel_.store(false);
    thread_ = std::thread(&Converter::convertThread, this, job, std::move(progress_cb));
}

std::vector<std::string> Converter::availableFormats() {
    return { "mp3", "flac", "wav", "ogg", "aac", "opus",
             "mp4", "mkv", "avi", "webm", "mov" };
}

// ─── Encoder capability helpers ───────────────────────────────────────────────
// FFmpeg 7.1 replaced AVCodec::sample_fmts / ch_layouts with
// avcodec_get_supported_config(); support both.
static AVSampleFormat preferredSampleFmt(const AVCodec* enc) {
    const AVSampleFormat* fmts = nullptr;
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(61, 13, 100)
    avcodec_get_supported_config(nullptr, enc, AV_CODEC_CONFIG_SAMPLE_FORMAT, 0,
                                 (const void**)&fmts, nullptr);
#else
    fmts = enc->sample_fmts;
#endif
    return (fmts && fmts[0] != AV_SAMPLE_FMT_NONE) ? fmts[0] : AV_SAMPLE_FMT_FLTP;
}

static void preferredChLayout(const AVCodec* enc, AVChannelLayout* out) {
    const AVChannelLayout* layouts = nullptr;
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(61, 13, 100)
    avcodec_get_supported_config(nullptr, enc, AV_CODEC_CONFIG_CHANNEL_LAYOUT, 0,
                                 (const void**)&layouts, nullptr);
#else
    layouts = enc->ch_layouts;
#endif
    // Prefer stereo if the encoder supports it (or doesn't restrict layouts)
    AVChannelLayout stereo = AV_CHANNEL_LAYOUT_STEREO;
    bool stereo_ok = !layouts;
    for (int i = 0; layouts && layouts[i].nb_channels; ++i)
        if (!av_channel_layout_compare(&layouts[i], &stereo)) stereo_ok = true;
    av_channel_layout_copy(out, stereo_ok ? &stereo : &layouts[0]);
}

// ─── Core conversion ─────────────────────────────────────────────────────────
void Converter::convertThread(ConvertJob job,
                               std::function<void(ConvertProgress)> cb) {
    ConvertProgress prog;

    struct StreamMap {
        int             in_idx   = -1;
        AVStream*       out_st   = nullptr;
        AVCodecContext* enc_ctx  = nullptr;
        AVCodecContext* dec_ctx  = nullptr;
        SwrContext*     swr      = nullptr;
        SwsContext*     sws      = nullptr;
        AVAudioFifo*    fifo     = nullptr;  // audio: re-chunks to the encoder's frame size
        int64_t         next_pts = 0;        // audio: running sample count
        bool            is_audio = false;
    };
    std::vector<StreamMap> maps;
    AVFormatContext* in_fmt  = nullptr;
    AVFormatContext* out_fmt = nullptr;
    AVPacket* pkt   = nullptr;
    AVFrame*  frame = nullptr;
    AVFrame*  conv  = nullptr;
    bool      output_created = false;

    auto cleanup = [&] {
        av_frame_free(&frame);
        av_frame_free(&conv);
        av_packet_free(&pkt);
        for (auto& sm : maps) {
            avcodec_free_context(&sm.dec_ctx);
            avcodec_free_context(&sm.enc_ctx);
            swr_free(&sm.swr);
            if (sm.sws)  sws_freeContext(sm.sws);
            if (sm.fifo) av_audio_fifo_free(sm.fifo);
        }
        maps.clear();
        if (out_fmt) {
            if (!(out_fmt->oformat->flags & AVFMT_NOFILE) && out_fmt->pb)
                avio_closep(&out_fmt->pb);
            avformat_free_context(out_fmt);
            out_fmt = nullptr;
        }
        avformat_close_input(&in_fmt);
    };
    auto fail = [&](const std::string& msg) {
        cleanup();
        // Don't leave a broken, half-written file behind
        if (output_created) { std::error_code ec; fs::remove(job.output_path, ec); }
        prog.error = true; prog.error_msg = msg; prog.done = true;
        cb(prog); running_.store(false);
    };

    // ── Open input ────────────────────────────────────────────────────────────
    if (avformat_open_input(&in_fmt, job.input_path.c_str(), nullptr, nullptr) < 0)
        return fail("Cannot open input: " + job.input_path);
    avformat_find_stream_info(in_fmt, nullptr);

    prog.duration = in_fmt->duration > 0
        ? (double)in_fmt->duration / AV_TIME_BASE : 0.0;

    // ── Open output ───────────────────────────────────────────────────────────
    if (avformat_alloc_output_context2(&out_fmt, nullptr, nullptr,
                                       job.output_path.c_str()) < 0 || !out_fmt)
        return fail("Cannot create output context (unknown output format?)");

    // ── Set up stream mappings ────────────────────────────────────────────────
    for (unsigned i = 0; i < in_fmt->nb_streams; ++i) {
        AVStream* in_st = in_fmt->streams[i];
        AVMediaType type = in_st->codecpar->codec_type;

        if (type == AVMEDIA_TYPE_AUDIO && job.strip_audio) continue;
        if (type == AVMEDIA_TYPE_VIDEO && job.strip_video) continue;
        if (type != AVMEDIA_TYPE_AUDIO && type != AVMEDIA_TYPE_VIDEO) continue;
        if (in_st->disposition & AV_DISPOSITION_ATTACHED_PIC) continue; // cover art

        StreamMap sm{};
        sm.in_idx   = (int)i;
        sm.is_audio = (type == AVMEDIA_TYPE_AUDIO);

        // Audio-only containers can't take a video stream
        std::string ext = fs::path(job.output_path).extension().string();
        std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
        bool audio_only_out = ext==".mp3" || ext==".flac" || ext==".wav" || ext==".ogg" ||
                              ext==".aac" || ext==".opus" || ext==".m4a";
        if (!sm.is_audio && audio_only_out) continue;

        // ── Decoder ──────────────────────────────────────────────────────────
        const AVCodec* dec = avcodec_find_decoder(in_st->codecpar->codec_id);
        if (!dec) continue;
        sm.dec_ctx = avcodec_alloc_context3(dec);
        avcodec_parameters_to_context(sm.dec_ctx, in_st->codecpar);
        sm.dec_ctx->thread_count = 2;
        sm.dec_ctx->pkt_timebase = in_st->time_base;
        if (avcodec_open2(sm.dec_ctx, dec, nullptr) < 0) {
            avcodec_free_context(&sm.dec_ctx); continue;
        }

        if (sm.is_audio) {
            // Determine output audio codec
            enum AVCodecID acodec_id = AV_CODEC_ID_MP3;
            if (ext==".flac")      acodec_id = AV_CODEC_ID_FLAC;
            else if (ext==".wav")  acodec_id = AV_CODEC_ID_PCM_S16LE;
            else if (ext==".ogg")  acodec_id = AV_CODEC_ID_VORBIS;
            else if (ext==".aac" || ext==".m4a") acodec_id = AV_CODEC_ID_AAC;
            else if (ext==".opus" || ext==".webm") acodec_id = AV_CODEC_ID_OPUS;
            else if (ext==".mp4" || ext==".mkv" || ext==".mov") acodec_id = AV_CODEC_ID_AAC;

            const AVCodec* enc = avcodec_find_encoder(acodec_id);
            if (!enc) {
                avcodec_free_context(&sm.dec_ctx);
                return fail(std::string("No encoder available for ") + avcodec_get_name(acodec_id));
            }
            sm.enc_ctx = avcodec_alloc_context3(enc);

            // Opus only supports 48 kHz
            sm.enc_ctx->sample_rate = acodec_id == AV_CODEC_ID_OPUS ? 48000 : sm.dec_ctx->sample_rate;
            sm.enc_ctx->bit_rate    = job.audio_bitrate;
            preferredChLayout(enc, &sm.enc_ctx->ch_layout);
            sm.enc_ctx->sample_fmt  = preferredSampleFmt(enc);
            sm.enc_ctx->time_base   = {1, sm.enc_ctx->sample_rate};

            if (out_fmt->oformat->flags & AVFMT_GLOBALHEADER)
                sm.enc_ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;

            if (avcodec_open2(sm.enc_ctx, enc, nullptr) < 0) {
                avcodec_free_context(&sm.dec_ctx);
                avcodec_free_context(&sm.enc_ctx);
                return fail(std::string("Cannot open encoder ") + enc->name);
            }

            // Resampler
            sm.swr = swr_alloc();
            av_opt_set_chlayout  (sm.swr, "in_chlayout",  &sm.dec_ctx->ch_layout, 0);
            av_opt_set_int       (sm.swr, "in_sample_rate",  sm.dec_ctx->sample_rate, 0);
            av_opt_set_sample_fmt(sm.swr, "in_sample_fmt",   sm.dec_ctx->sample_fmt, 0);
            av_opt_set_chlayout  (sm.swr, "out_chlayout", &sm.enc_ctx->ch_layout, 0);
            av_opt_set_int       (sm.swr, "out_sample_rate", sm.enc_ctx->sample_rate, 0);
            av_opt_set_sample_fmt(sm.swr, "out_sample_fmt",  sm.enc_ctx->sample_fmt, 0);
            if (swr_init(sm.swr) < 0) {
                avcodec_free_context(&sm.dec_ctx);
                avcodec_free_context(&sm.enc_ctx);
                swr_free(&sm.swr);
                return fail("Cannot initialise audio resampler");
            }
            sm.fifo = av_audio_fifo_alloc(sm.enc_ctx->sample_fmt,
                                          sm.enc_ctx->ch_layout.nb_channels, 4096);

        } else {
            // Video: copy stream (no re-encode for speed/quality, unless resize requested)
            bool needs_reencode = (job.width > 0 || job.height > 0);
            if (!needs_reencode) {
                avcodec_free_context(&sm.dec_ctx);
            } else {
                const AVCodec* enc = avcodec_find_encoder(AV_CODEC_ID_H264);
                if (!enc) {
                    avcodec_free_context(&sm.dec_ctx);
                    return fail("No H.264 encoder available in this FFmpeg build");
                }
                sm.enc_ctx = avcodec_alloc_context3(enc);
                sm.enc_ctx->width     = job.width  > 0 ? job.width  : sm.dec_ctx->width;
                sm.enc_ctx->height    = job.height > 0 ? job.height : sm.dec_ctx->height;
                sm.enc_ctx->pix_fmt   = AV_PIX_FMT_YUV420P;
                sm.enc_ctx->bit_rate  = job.video_bitrate;
                sm.enc_ctx->time_base = in_st->time_base;
                sm.enc_ctx->framerate = av_guess_frame_rate(in_fmt, in_st, nullptr);
                if (out_fmt->oformat->flags & AVFMT_GLOBALHEADER)
                    sm.enc_ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
                if (avcodec_open2(sm.enc_ctx, enc, nullptr) < 0) {
                    avcodec_free_context(&sm.dec_ctx);
                    avcodec_free_context(&sm.enc_ctx);
                    return fail(std::string("Cannot open encoder ") + enc->name);
                }
                sm.sws = sws_getContext(
                    sm.dec_ctx->width, sm.dec_ctx->height, sm.dec_ctx->pix_fmt,
                    sm.enc_ctx->width, sm.enc_ctx->height, AV_PIX_FMT_YUV420P,
                    SWS_BILINEAR, nullptr, nullptr, nullptr);
            }
        }

        // Output stream (created only once the stream is known to be usable)
        sm.out_st = avformat_new_stream(out_fmt, nullptr);
        if (sm.enc_ctx) {
            avcodec_parameters_from_context(sm.out_st->codecpar, sm.enc_ctx);
            sm.out_st->time_base = sm.enc_ctx->time_base;
        } else {
            avcodec_parameters_copy(sm.out_st->codecpar, in_st->codecpar);
            sm.out_st->codecpar->codec_tag = 0;
            sm.out_st->time_base = in_st->time_base;
        }
        maps.push_back(sm);
    }

    if (maps.empty()) return fail("No usable streams found");

    // ── Open output file ──────────────────────────────────────────────────────
    if (!(out_fmt->oformat->flags & AVFMT_NOFILE)) {
        if (avio_open(&out_fmt->pb, job.output_path.c_str(), AVIO_FLAG_WRITE) < 0)
            return fail("Cannot open output file: " + job.output_path);
        output_created = true;
    }
    if (avformat_write_header(out_fmt, nullptr) < 0)
        return fail("Cannot write output header (codec not supported by this container?)");

    // ── Transcode loop ────────────────────────────────────────────────────────
    pkt   = av_packet_alloc();
    frame = av_frame_alloc();
    conv  = av_frame_alloc();

    // Send a frame (or nullptr to flush) to the encoder and mux what comes out
    auto encodeAndWrite = [&](StreamMap& sm, AVFrame* f) {
        avcodec_send_frame(sm.enc_ctx, f);
        while (avcodec_receive_packet(sm.enc_ctx, pkt) == 0) {
            pkt->stream_index = sm.out_st->index;
            av_packet_rescale_ts(pkt, sm.enc_ctx->time_base, sm.out_st->time_base);
            av_interleaved_write_frame(out_fmt, pkt);
        }
    };

    // Audio encoders need fixed-size frames: feed them from the FIFO
    auto drainAudioFifo = [&](StreamMap& sm, bool final) {
        const bool variable = sm.enc_ctx->frame_size <= 0 ||
            (sm.enc_ctx->codec->capabilities & AV_CODEC_CAP_VARIABLE_FRAME_SIZE);
        const int  fs_ = variable ? 1024 : sm.enc_ctx->frame_size;
        while (av_audio_fifo_size(sm.fifo) >= fs_ ||
               (final && av_audio_fifo_size(sm.fifo) > 0)) {
            int n = std::min(fs_, av_audio_fifo_size(sm.fifo));
            av_frame_unref(conv);
            conv->format      = sm.enc_ctx->sample_fmt;
            conv->sample_rate = sm.enc_ctx->sample_rate;
            av_channel_layout_copy(&conv->ch_layout, &sm.enc_ctx->ch_layout);
            // A short last frame is padded with silence unless the encoder accepts it
            bool pad = !variable && n < fs_ &&
                !(sm.enc_ctx->codec->capabilities & AV_CODEC_CAP_SMALL_LAST_FRAME);
            conv->nb_samples = pad ? fs_ : n;
            if (av_frame_get_buffer(conv, 0) < 0) return;
            if (pad) av_samples_set_silence(conv->data, 0, fs_,
                         conv->ch_layout.nb_channels, (AVSampleFormat)conv->format);
            av_audio_fifo_read(sm.fifo, (void**)conv->data, n);
            conv->pts = sm.next_pts;
            sm.next_pts += conv->nb_samples;
            encodeAndWrite(sm, conv);
        }
    };

    // Resample one decoded frame (or flush the resampler with nullptr) into the FIFO
    auto pushAudio = [&](StreamMap& sm, AVFrame* in) {
        int max_out = (int)av_rescale_rnd(
            swr_get_delay(sm.swr, sm.dec_ctx->sample_rate) + (in ? in->nb_samples : 0),
            sm.enc_ctx->sample_rate, sm.dec_ctx->sample_rate, AV_ROUND_UP);
        if (max_out <= 0) return;
        uint8_t** buf = nullptr;
        if (av_samples_alloc_array_and_samples(&buf, nullptr, sm.enc_ctx->ch_layout.nb_channels,
                                               max_out, sm.enc_ctx->sample_fmt, 0) < 0) return;
        int got = swr_convert(sm.swr, buf, max_out,
            in ? (const uint8_t**)in->data : nullptr, in ? in->nb_samples : 0);
        if (got > 0) av_audio_fifo_write(sm.fifo, (void**)buf, got);
        av_freep(&buf[0]);
        av_freep(&buf);
    };

    while (!cancel_.load() && av_read_frame(in_fmt, pkt) >= 0) {
        int si = pkt->stream_index;
        // Find matching map
        StreamMap* sm = nullptr;
        for (auto& m : maps) if (m.in_idx == si) { sm = &m; break; }
        if (!sm) { av_packet_unref(pkt); continue; }

        // Update progress
        if (pkt->pts != AV_NOPTS_VALUE) {
            double pts_s = pkt->pts * av_q2d(in_fmt->streams[si]->time_base);
            prog.current_time = pts_s;
            prog.progress     = prog.duration > 0 ? std::min(1.0, pts_s / prog.duration) : 0.0;
            cb(prog);
        }

        // If no decoder (stream copy), just mux the packet directly
        if (!sm->dec_ctx) {
            av_packet_rescale_ts(pkt,
                in_fmt->streams[si]->time_base, sm->out_st->time_base);
            pkt->stream_index = sm->out_st->index;
            av_interleaved_write_frame(out_fmt, pkt);
            continue;
        }

        avcodec_send_packet(sm->dec_ctx, pkt);
        av_packet_unref(pkt);

        while (avcodec_receive_frame(sm->dec_ctx, frame) == 0) {
            if (sm->is_audio) {
                pushAudio(*sm, frame);
                drainAudioFifo(*sm, false);
            } else if (sm->sws) {
                // Scale video
                av_frame_unref(conv);
                conv->format = AV_PIX_FMT_YUV420P;
                conv->width  = sm->enc_ctx->width;
                conv->height = sm->enc_ctx->height;
                av_frame_get_buffer(conv, 0);
                sws_scale(sm->sws,
                    (const uint8_t* const*)frame->data, frame->linesize,
                    0, sm->dec_ctx->height,
                    conv->data, conv->linesize);
                conv->pts = frame->best_effort_timestamp;
                encodeAndWrite(*sm, conv);
            }
            av_frame_unref(frame);
        }
    }

    // Flush decoders, resamplers and encoders
    if (!cancel_.load()) {
        for (auto& sm : maps) {
            if (!sm.enc_ctx) continue;
            avcodec_send_packet(sm.dec_ctx, nullptr);
            while (avcodec_receive_frame(sm.dec_ctx, frame) == 0) {
                if (sm.is_audio) pushAudio(sm, frame);
                av_frame_unref(frame);
            }
            if (sm.is_audio) {
                pushAudio(sm, nullptr);
                drainAudioFifo(sm, true);
            }
            encodeAndWrite(sm, nullptr);
        }
    }
    av_write_trailer(out_fmt);
    cleanup();

    if (!cancel_.load()) {
        prog.progress = 1.0; prog.done = true;
        cb(prog);
    }
    running_.store(false);
}
